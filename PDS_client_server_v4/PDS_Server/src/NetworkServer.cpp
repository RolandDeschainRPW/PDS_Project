...
void NetworkServer::writeStartDataToClient(QTcpSocket* clientConnection, bool new_document, QString filename, QString owner_username, QString opener_nickname) {
    ...
    if (new_document) {
        ...
        connected_editors = this->getConnectedCollaborators(doc, site_id_assigned);
...
QVector<Collaborator> NetworkServer::getConnectedCollaborators(SharedDocument* document, qint32 site_id) {
    ...
    foreach (CollaboratorPair pair, connected_editors) {
        ...
        // Searching cursor position.
        qint32 cursor_pos = document->getSharedEditor(site_id)->getCursorPos();

        collaborators.push_back(Collaborator(pair.second, profile_pic, pair.first, cursor_pos));
    }
