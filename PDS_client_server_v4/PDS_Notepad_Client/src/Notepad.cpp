...
Notepad::Notepad(NetworkingData* net_data,
    ...
    // Updating profile pics and cursors on UI.
    ...
    foreach (Collaborator col, connected_editors) {
        this->updateConnectedCollaborators(col.getSiteId(),
                                           ...
                                           col.getNickname(),
                                           col.getProfilePic());
        if (col.getSiteId() != net_data->getSiteId()) {
            ui->textEdit->addCursor(col.getSiteId());
            ui->textEdit->setCursorPosition(col.getSiteId(), col.getCursorPos());
        }
    }
...
void Notepad::onCursorPositionChanged() {
    ...
    Request req_to_send = Request(this->net_data->getSiteId(),
                                  Request::MESSAGE_TYPE,
                                  Message(this->net_data->getSiteId(),
                                          Message::CURSOR_POS_CHANGED_TYPE,
                                          std::nullopt,
                                          std::nullopt,
                                          "", ui->textEdit->textCursor().position()),
                                  "", "", net_data->getRemoteFilePath());
...
void Notepad::readMessage(Message& msg) {
    if (msg.getType() == Message::INSERT_TYPE || msg.getType() == Message::ERASE_TYPE)
        ...
        ui->textEdit->addCursor(msg.getSiteId());
    } else if (msg.getType() == Message::CURSOR_POS_CHANGED_TYPE) {
        ui->textEdit->setCursorPosition(msg.getSiteId(), msg.getCursorPosition());
    } else /* Message::NOTIFY_DISCONNECTION */ {
        this->updateConnectedCollaborators(msg.getSiteId(),
                                           msg.getType());
        ui->textEdit->removeCursor(msg.getSiteId());
    }
}
