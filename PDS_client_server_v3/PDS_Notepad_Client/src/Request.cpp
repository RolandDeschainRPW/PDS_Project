...
Request::Request(qint32 siteId,
                 ...
                 QString username,
                 QString password) : siteId(siteId),
...
QDataStream &operator<<(QDataStream &out, const Request& req) {
    out << req.getSiteId() << req.getRequestType() << req.getUsername() << req.getPassword();
...
QDataStream &operator>>(QDataStream &in, Request& req) {
    ...
    in >> siteId >> request_type >> username >> password;
    if (request_type == Request::MESSAGE_TYPE) {
        in >> msg;
        req = Request(siteId, request_type, msg, username, password);
        return in;
    }
    req = Request(siteId, request_type, std::nullopt, username, password);
    return in;
}
